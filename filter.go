...
func (r *pluginContext) OnPluginStart(pluginConfigurationSize int) types.OnPluginStartStatus {
...
// Override types.DefaultPluginContext.
func (r *pluginContext) NewHttpContext(contextID uint32) types.HttpContext {
	return &payloadValidationContext{requiredKeys: r.configuration.requiredKeys}
}
...
// Override types.DefaultHttpContext.
func (r *payloadValidationContext) OnHttpRequestBody(bodySize int, endOfStream bool) types.Action {
	r.totalRequestBodySize += bodySize
	if !endOfStream {
	...
	body, err := proxywasm.GetHttpRequestBody(0, r.totalRequestBodySize)
	if err != nil {
		proxywasm.LogErrorf("failed to get request body: %v", err)
		return types.ActionContinue
	}
	if !r.validatePayload(body) {
...
func (r *payloadValidationContext) validatePayload(body []byte) bool {
	...
	for _, requiredKey := range r.requiredKeys {
